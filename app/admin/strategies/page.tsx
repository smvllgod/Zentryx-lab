  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-4">
        ...
        <StatCard label="Drafts" value={drafts} tone="default" />
        ...
        columns={[
          {
            ...
            render: (r) => (
              <div className="min-w-0">
                <div className="text-sm font-600 text-gray-900 truncate">{r.name}</div>
                <div className="text-[10px] text-gray-400 truncate">{r.id}</div>
              </div>
            ),
          },
          { header: "Status", width: "110px", render: (r) => <Badge tone={r.status === "published" ? "purple" : r.status === "exported" ? "emerald" : "slate"}>{r.status}</Badge> },
          ...
          { header: "Owner", width: "130px", render: (r) => <a href={`/admin/users/${r.user_id}`} className="text-[10px] font-600 text-emerald-600 hover:underline">{r.user_id.slice(0, 8)}…</a> },
          { header: "Updated", width: "120px", render: (r) => <span className="text-xs text-gray-500">{formatRelative(r.updated_at)}</span> },
        ]}
      />
    </AdminShell>
  );
